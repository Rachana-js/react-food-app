...
const Cart = () => {
  ...
  return (
    <motion.div {...slideIn} className='fixed z-50 top-0 right-0 w-300 md:w-460 bg-lightOverlay backdrop-blur-md shadow-md h-screen'>
        <div className='w-full flex items-center justify-between py-4 pb-10 px-6'>
        <motion.i
          {...buttonClick}
        ...
        <div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6  gap-3 relative">
           {cart && cart?.length > 0 ?( <>
            <div className="flex flex-col w-full items-start justify-start gap-1 h-[65%] overflow-y-scroll scrollbar-none px-4">
            {cart &&
                cart?.length > 0 &&
                cart?.map((item, i) => (
            ...
            <div className="bg-zinc-800 rounded-t-[60px] w-full h-[40%] flex flex-col items-center justify-start px-4 py-6 gap-1">
...
export const CartItemCard =({index,data})=>{
    ...
    return(
        <motion.div key={data.productId} {...staggerFadeInOut(index)}
